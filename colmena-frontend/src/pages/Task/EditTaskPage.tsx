...
export function EditTaskPage() {
  ...
  const onSubmit = async (data: EditTaskFormData) => {
    try {
      ...
      navigate(`/tasks/task-details/${id}`);
    } catch (error) {
      if (error instanceof AxiosError) {
        setError("root", {
          message: error.response?.data.message || "Could not update task",
        });
      } else {
        setError("root", { message: "Could not update task" });
      }
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError("root", { message: "Unable to delete task" });
    }
  };
